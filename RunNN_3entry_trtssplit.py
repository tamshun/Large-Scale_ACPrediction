...
class Classification(Base_wodirection):
    ...
    def _AllMMSPred(self, target):
        ...
        if self._IsPredictableTarget():
            ...
            for trial in [2]:#self.testsetidx:    
                ...
                tr, ts, df_trX, df_trY, df_tsX, df_tsY, trX, trY, tsX, tsY = self._GetMatrices(trial, separated_input=True)
                ...
                if self._IsPredictableSeries(tr, ts, min_npos=self.nfold):
                    ...
                    score_ts, predY_ts, proba_ts   = self._predict_bestparams(dnn_c, dnn_s1, dnn_s2, dnn_cat, args, tsX, tsY, loss_fn)
                    print("    $ Prediction Done.\n")

                    # Write & save log
                    log_tr = self.WriteLog_tr(trial, tr, trY, predY_tr, proba_tr) 
                    log_ts = self.WriteLog_ts(trial, tr, ts, tsY, predY_ts, proba_ts)  
                    self.Save(target, trial, log_tr, log_ts, args_c, args_s, args_cat, dnn_c, dnn_s1, dnn_s2, dnn_cat)
                    print("    $  Log is out.\n")      
    ...
    def Save(self, target, seed, log_tr, log_ts, args_c, args_s, args_cat, dnn_c, dnn_s1, dnn_s2, dnn_cat):
        path_log_tr = os.path.join(self.logdir, "%s_trial%d_train.tsv" %(target, seed))
        self.log_tr = pd.DataFrame.from_dict(log_tr)
        self.log_tr.to_csv(path_log_tr, sep="\t")
        
        path_log_ts = os.path.join(self.logdir, "%s_trial%d_test.tsv" %(target, seed))
        ...
        ToJson(args, self.modeldir+"/params_%s_trial%d.json" %(target, seed))
        
        torch.save(dnn_c.to('cpu').state_dict() ,  self.modeldir+"/dnn_c_%s_trial%d.pth" %(target, seed))
        torch.save(dnn_s1.to('cpu').state_dict(),  self.modeldir+"/dnn_s1_%s_trial%d.pth" %(target, seed))
        torch.save(dnn_s2.to('cpu').state_dict(),  self.modeldir+"/dnn_s2_%s_trial%d.pth" %(target, seed))
        torch.save(dnn_cat.to('cpu').state_dict(), self.modeldir+"/dnn_cat_%s_trial%d.pth" %(target, seed))
