...
class Classification(Base_wodirection_CGR):
    ...
    def __init__(self, modeltype, model, dir_log, dir_score, data_split_metric='trtssplit', debug=False):
    ...
    def Save(self, target, cid, log_tr, log_ts, args_mpn_c, args_mpn_s, args_dnn, mpn_c, mpn_s1, mpn_s2, dnn):
        ...
        torch.save(mpn_c.to('cpu').state_dict() , self.modeldir+"/mpn_c_%s_trial%d.pth" %(target, cid))
        torch.save(mpn_s1.to('cpu').state_dict(), self.modeldir+"/mpn_s1_%s_trial%d.pth" %(target, cid))
        torch.save(mpn_s2.to('cpu').state_dict(), self.modeldir+"/mpn_s2_%s_trial%d.pth" %(target, cid))
    ...
    if not p._IsPredictableSet():
        ...
        if p._IsPredictableTarget():
            ...
            for loop in p.testsetidx:    
                ...
                tr, ts = p._GetTrainTest(loop, unbiased=True)
                trX, trY = tr[cols], tr['class'].to_numpy()
                tsX, tsY = ts[cols], ts['class'].to_numpy()
                ...
                if p._IsPredictableSeries(tr, ts, min_npos=p.nfold):
                    ...
                    score_ts, predY_ts, proba_ts = predict_bestparams(info, mpn_c, mpn_s1, mpn_s2, dnn, args_dnn, tsX, tsY, loss_fn)
                    print("    $ Prediction Done.\n")
                    
                    # Write & save log
                    log_tr = p.WriteLog_tr(loop, tr, trY, predY_tr, proba_tr) 
                    log_ts = p.WriteLog_ts(loop, tr, ts, tsY, predY_ts, proba_ts)  
                    p.Save(target, loop, log_tr, log_ts, args_mpn_c, args_mpn_s, args_dnn, mpn_c, mpn_s1, mpn_s2, dnn)
                    print("    $  Log is out.\n")
...
if __name__ == '__main__':    
    ...
    # main('CHEMBL204', bd, debug=True)
    
    obj = partial(main, bd=bd, debug=debug)
    joblib.Parallel(n_jobs=10, backend='loky')(joblib.delayed(obj)(target) for target in tlist)