...
class Classification(Base_wodirection):
        ...
        if self._IsPredictableTarget():    
            ...
            for cid in self.testsetidx:    
                ...
                if self._IsPredictableSeries(tr, ts, min_npos=self.nfold):
                    ...
                    # Write & save log
                    log = self._WriteLog(log, ml, cid, tr, ts, tsX, tsY, predY)           
                    self._Save(target, cid, log, ml)
                    print("    $  Log is out.\n")
                else:
                    print('    $ cid%d is not predictable' %cid)
    ...
    def _Save(self, target, cid, log, ml):
        ...
        path_log = os.path.join(self.logdir, "%s.tsv" %(target))
        ...
        ToJson(params, self.modeldir+"/%s_%d.json"%(target, cid))
